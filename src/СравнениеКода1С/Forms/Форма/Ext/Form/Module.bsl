...
Процедура УстановитьОтборСтрокСравнения()
	СтруктураОтбор = Новый Структура("КлючМетода", "БезОтбора");
	
	ТекущиеДанные = Элементы.СтруктураМодуля.ТекущиеДанные;
	Если НЕ ТекущиеДанные = Неопределено И НЕ ПустаяСтрока(ТекущиеДанные.КлючМетода) Тогда
...
Процедура ОбновитьСтруктуруМодуля()
	...
	Для Каждого СтрокаОбласти Из ЭтотОбъект.Области Цикл
		Если НЕ ПустаяСтрока(СтрокаОбласти.ИмяРодитель1)
			И ВсеРодители.Найти(СтрокаОбласти.ИмяРодитель1) = Неопределено Тогда
			ВсеРодители.Добавить(СтрокаОбласти.ИмяРодитель1);
		КонецЕсли;
		
		Если НЕ ПустаяСтрока(СтрокаОбласти.ИмяРодитель2)
			И ВсеРодители.Найти(СтрокаОбласти.ИмяРодитель2) = Неопределено Тогда
			ВсеРодители.Добавить(СтрокаОбласти.ИмяРодитель2);
		КонецЕсли;
	КонецЦикла;
	...
	Для Каждого СтрокаОбласти Из ЭтотОбъект.Области Цикл
		...
		ЭтоРодитель = (ЗначениеЗаполнено(СтрокаОбласти.ПолноеИмя1) И ВсеРодители.Найти(СтрокаОбласти.ПолноеИмя1) <> Неопределено)
			ИЛИ (ЗначениеЗаполнено(СтрокаОбласти.ПолноеИмя2) И ВсеРодители.Найти(СтрокаОбласти.ПолноеИмя2) <> Неопределено);
			
		Если ЭтоРодитель Тогда
			КэшРодителей.Вставить(СтрокаОбласти.ПолноеИмя1, СтрокаОбласти);
			КэшРодителей.Вставить(СтрокаОбласти.ПолноеИмя2, СтрокаОбласти);
		КонецЕсли;
		
		МассивМетодов = КэшМетодов.Получить(СтрокаОбласти.КлючОбласти);
		...
		Если ЭтотОбъект.ВидСтруктурыМодуля = 0 Тогда
			Если ЗначениеЗаполнено(СтрокаОбласти.ИмяРодитель1) Тогда
				ОбластьРодитель = КэшРодителей.Получить(СтрокаОбласти.ИмяРодитель1);
			ИначеЕсли ЗначениеЗаполнено(СтрокаОбласти.ИмяРодитель2) Тогда
				ОбластьРодитель = КэшРодителей.Получить(СтрокаОбласти.ИмяРодитель2);
			Иначе 
				ОбластьРодитель = Неопределено;
			КонецЕсли;
			
			Если ОбластьРодитель <> Неопределено Тогда
				СтрокаРодитель = КэшОбластей.Получить(ОбластьРодитель.КлючОбласти);
			Иначе 
			...
			СтрокаДерева.КлючОбласти1	= СтрокаОбласти.КлючОбласти;
			СтрокаДерева.КлючОбласти2	= СтрокаОбласти.КлючОбласти;
			СтрокаДерева.КлючМетода		= "-";
		Иначе 
		...
		КэшОбластей.Вставить(СтрокаОбласти.КлючОбласти, СтрокаДерева);
	КонецЦикла;
	
	Если БезОбластей.Количество() > 0 Тогда
		Если ЭтотОбъект.ВидСтруктурыМодуля Тогда
			СтрокаДерева = ЭтотОбъект.СтруктураМодуля.ПолучитьЭлементы().Добавить();
			СтрокаДерева.Идентификатор	= "";
			СтрокаДерева.Имя1			= НСтр("ru='Без области'");
			СтрокаДерева.Знак1			= "";
			СтрокаДерева.Строки1		= "";
			СтрокаДерева.Имя2			= НСтр("ru='Без области'");
			СтрокаДерева.Знак2			= "";
			СтрокаДерева.Строки2		= "";
			СтрокаДерева.ЭтоОбласть		= Истина;
			СтрокаДерева.ЭтоМетод		= Ложь;
			
			СтрокаДерева.КлючОбласти1	= "-";
			СтрокаДерева.КлючОбласти2	= "-";
			СтрокаДерева.КлючМетода		= "-";
		Иначе 
			СтрокаДерева = ЭтотОбъект.СтруктураМодуля;
		КонецЕсли;
...
Процедура ЗаполнитьСтрокуСтруктурыМодуляИзМетода(знач СтрокаВложенная, знач СтрокаМетода)
	...
	СтрокаВложенная.КлючМетода		= СтрокаМетода.КлючМетода;
...
Процедура ЗаполнитьСтрокиОбластей(знач Форма, знач СтрокаМетода)
	...
	СтрокаМетода.КлючОбласти1	= ?(Область1 = Неопределено, "", Область1.КлючОбласти);
	СтрокаМетода.КлючОбласти2	= ?(Область2 = Неопределено, "", Область2.КлючОбласти);
КонецПроцедуры
...
Процедура ЗаполнитьМассивОбластей(знач Форма, знач Массив, знач Сторона)
	...
	ЗаголовокСостояния = СтрШаблон(НСтр("ru='Поиск областей (файл %1)'"), Сторона);
	...
	Для НомерСтроки = НачСтроки По КонСтроки Цикл
		...
		Если ДанныеБлока.Открыт Тогда
			Если ЭтоОкончаниеБлока(СтрокаАнализ, ДанныеБлока.КонИмяБлока) Тогда
				ДанныеОбласти = Новый Структура("Имя,НачСтрока,НачПозиции,КонСтрока,КонПозиции,ИмяРодитель,ПолноеИмя",
					ДанныеБлока.ИмяБлока,
					ДанныеБлока.НачСтроки,
					ДанныеБлока.НачПозиции,
					НомерСтроки,
					КонПозиции);
					
				ДанныеОбласти.ИмяРодитель = ДанныеБлока.ПутьКБлоку;
				
				ДанныеОбласти.ПолноеИмя = ДанныеБлока.ПутьКБлоку
					+ ?(ПустаяСтрока(ДанныеБлока.ПутьКБлоку), "", "/")
					+ ДанныеОбласти.Имя;
					
				ДобавитьДанныеМодуляВМассив(Массив, ДанныеОбласти);
...
&НаКлиентеНаСервереБезКонтекста 
Функция ПолучитьИмяОбластиРодитель(знач ОткрытыеБлоки)
	ИмяРодителя = Новый Массив;
	Для Каждого ДанныеБлока Из ОткрытыеБлоки Цикл
		ИмяРодителя.Добавить(ДанныеБлока.ИмяБлока);
	КонецЦикла;
	
	Возврат СтрСоединить(ИмяРодителя, "/");
КонецФункции
...
Процедура ЗаполнитьМассивМетодов(знач Форма, знач Массив, знач Сторона)
	...
	ЗаголовокСостояния = СтрШаблон(НСтр("ru='Поиск методов (файл %1)'"), Сторона);
	ПараметрыСостояния = ПолучитьПараметрыСостояния(ЗаголовокСостояния, КонСтроки);
	...
	Для НомерСтроки = НачСтроки По КонСтроки Цикл
		...
		Для Каждого КлючИЗначение Из ИменаБлоков Цикл
			...
			Если СтрНачинаетсяС(СтрокаАнализ, ИмяНачало) Тогда
				...
				ДанныеБлока.НачИмяБлока	= ИмяНачало;
				ДанныеБлока.КонИмяБлока	= ИмяКонец;
				ДанныеБлока.ИмяБлока	= ВыделитьИмяМетода(СтрокаМодуля, ИмяНачало);
				ДанныеБлока.НачСтроки	= НомерСтроки;
				ДанныеБлока.НачПозиции	= НачПозиции;
		...
		Если ДанныеБлока.Открыт Тогда
			Если ЭтоОкончаниеБлока(СтрокаАнализ, ДанныеБлока.КонИмяБлока) Тогда
				ДанныеМетода = Новый Структура("Имя,НачСтрока,НачПозиции,КонСтрока,КонПозиции,ЭтоАсинх,ЭтоФункция",
					ДанныеБлока.ИмяБлока,
					ДанныеБлока.НачСтроки,
					ДанныеБлока.НачПозиции,
					НомерСтроки,
				...
				ДанныеМетода.ЭтоФункция = (СтрКонецФункции.Найти(ДанныеБлока.КонИмяБлока) <> Неопределено);
					
				ДополнитьДанныеМетода(ТекстМодуля, ТекстАнализ, ДанныеМетода);
					
				ДобавитьДанныеМодуляВМассив(Массив, ДанныеМетода);
...
&НаКлиентеНаСервереБезКонтекста 
Функция ВыделитьИмяМетода(знач ТекстМетода, знач ИмяБлока)
