...
Процедура ПриСозданииНаСервере(Отказ, СтандартнаяОбработка)
	...
	ЭтотОбъект.Заголовок = СтрШаблон(НСтр("ru='Сравнение текстов модулей (версия %1)'"), НомерВерсии());
...
Процедура ЗаполнитьМассивОбластей(знач Форма, знач Массив, знач Сторона)
	...
	ПараметрыСостояния = ПолучитьПараметрыСостояния(ЗаголовокСостояния, ЧислоСтрок);
...
Процедура ЗаполнитьМассивМетодов(знач Форма, знач Массив, знач Сторона)
	...
	Для Индекс = НачСтроки По КонСтроки Цикл
		...
		Для ИндексПодстроки = 0 По МассивНачало.ВГраница() Цикл
			...
			Структура = Новый Структура("Имя,НачСтрока,НачПозиции,КонСтрока,КонПозиции,КонецМетода");
